...
> 3. For $s<t$ the random variable $W_t-W_s$ has the Gaussian distribution <span class="math inline"><img src="https://math.now.sh?inline=\mathcal{N}(0,t-s)" /></span>.
...
Furthermore, for $r<s\le t< u$ it holds that $W_u-W_t$ and $W_s-W_r$ is independent. By seperate transformations the independence property is preserved and $-(W_u-W_t)$ and $-(W_s-W_r)$ is independent. Next, for a normal distributed random variable $N\sim$<span class="math inline"><img src="https://math.now.sh?inline=N\sim\mathcal{N}(\mu,\sigma^2)"/></span> it holds, that for a scaler $c\in\mathbb{R}$ we have <span class="math inline"><img src="https://math.now.sh?inline=c N\sim\mathcal{N}(c\mu,c^2\sigma ^2)" /></span>. Then obviously;

<span class="math center"><img src="https://math.now.sh?inline=-(W_t)=(-1)W_t\stackrel{d}{=}\mathcal{N}((-1)\cdot0,(-1)^2(t-s))\stackrel{d}{=}\mathcal{N}( 0,t-s)."/></span>

Lastly, let $\omega \in \Omega$ and consider the sample path $s\mapsto (-W_s)(\omega)$. Clearly for two continuous functions $f$ and $g$ it holds that $(g\circ f)$ is continuous. Then with $g(x)=-x$ and $f(t)=W_t(\omega)$ $(-W_t)=(g\circ W)(t)$ is also continuous.
...
$$X_u-X_r=W(u+s)-W(s)-(W(r+s)-W(s))=W(u+s)-W(r+s)\sim N(0,u+s-(r+s))=N(0,u-r).$$

and since for $r<u\le k<l$ the translation $r+s<u+s\le k+s<l+s$ still holds and $X_l-X_k=W(l+s)-W(k+s)$ and $X_u-X_r=W(u+s)-W(k+s)$ are independent.

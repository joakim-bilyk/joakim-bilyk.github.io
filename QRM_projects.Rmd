---
title: "Projects in Quantitative Risk Management"
output:
  html_document:
    toc: true
    toc_float: true
...
<iframe src=
"pdf/render/QRM-Assignment1.pdf" 
                width="800" 
                height="1100">
</iframe>
...
<iframe src=
"pdf/render/QRM-Assignment2.pdf" 
                width="800" 
                height="1100">
</iframe>
...
#Look page 80 for R_i,j; rho_tau is given in problem-text, and diag is just 1 since X is continous hence the probabilities are 1 and 0 
...
$$
VaR_{0.99}\approx8.55, \hspace{30pt} ES_{0.99}\approx11.74
$$
...
var(dax_returns[,2])
var(bmw_returns[,2])
sigma <- matrix(c(cov(dax_returns[,2],bmw_returns[,2]), var(dax_returns[,2]),cov(dax_returns[,2],bmw_returns[,2]), var(bmw_returns[,2])), nrow = 2)
S_n <- 100
...
We compute that $VaR_{0.99}$ is 5.37. Which is significantly smaller than the estimate computed in the previous problem. This might be because the estimate $e^X\approx 1+x$ underestimates our losses. As discussed in the previous problem the t-distribution fits best to the data, which is also a problem, since we assume a normal distribution in the standard variance-covariance model. The normal distribution has lighter tails than the t distribution. This means the model might not catch the extreme events very well leading to a more moderate $VaR$ estimate.
...
$$
    \tilde{C}(a_1,a_2,b_3)-\tilde{C}(a_1,a_2,a_3)+\tilde{C}(a_1,b_2,a_3)+\tilde{C}(b_1,a_2,a_3)+\tilde{C}(b_1,b_2,b_3)
...
$$
\alpha =\mathbf{P}\left\{L_n>na\right\}\approx e^{-n\Lambda^*(a)},\hspace{20pt} \Lambda^*(a)=\xi_{sup}(a)\cdot a-\Lambda(\xi_{sup}(a)),
$$
...
However since $\Lambda^*$ is convex it and continuous is does not have an inverse. We see that the derivative is given by

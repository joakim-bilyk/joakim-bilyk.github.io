...
\addcontentsline{toc}{section}{To-do reading}
...
\begin{longtable}[]{@{}
  >{\centering\arraybackslash}p{(\columnwidth - 2\tabcolsep) * \real{0.3000}}
...
Then \(S_t\) must be the value at time \(t\) of a unit deposited at time 0, and we call it the \emph{accumulation function}\index{accumulation function}. Correspondingly, \(S_t^{-1}\) is the value at time 0 of a unit withdrawn at time \(t\), and we call it the \emph{discount function}\index{discount function}.
...
In survival analysis it is convenient to work with the \emph{survival function}\index{survival function}
...
\textbf{Definition 1.1. (Lund)} \emph{A right-continuous \(\mathbb R\)-valued function \(x(t)\) with finite left-limits defined on \([0,\infty)\) is called CADLAG\index{CADLAG} if}
...
\emph{A left-continuous function on \((0,\infty)\) with right-limits on \([0, \infty)\) is called CAGLAD\index{CAGLAD}.}
...
\textbf{Definition 2.2. (Lund) (Variation)}\index{Variation} \emph{The variation of the function \(x : [0, \infty) \to\mathbb R\) on the interval \([0, t]\) is defined by}
...
\textbf{Lemma 2.5. (Lund) (Jordan decomposition)}\index{Jordan decomposition} \emph{Let \(x\) be af function of finite variation. Then}
...
\textbf{Definition 2.8. (Lund) (FV-functions)}\index{FV-functions} \emph{A function \(x : [0,\infty) \to \mathbb R\) is an FV-function if }\(x(t)\) is of finite variation and is CADLAG.\_
...
\textbf{Proposition 2.12. (Lund) (Lebesgue decomposition of function of finite variation)}\index{Lebesgue decomposition} \emph{An FV-function \(x\) can be written as}
...
\textbf{Definition 3.1. (Lund) (Lebesgue-Stieltjes measure)}\index{Lebesgue-Stieltjes measure} \emph{A measure \(\mu\) on \(((0,\infty),\mathcal B(0,\infty))\) is a Lebesgue-Stieltjes measure if \(\mu(A)<\infty\) for every bounded interval \(A\subseteq (0,\infty)\).}
...
\textbf{Definition 3.4. (Lund) (Lebesgue-Stieltjes integral)}\index{Lebesgue-Stieltjes integral} \emph{Let \(a : [0,\infty) \to \mathbb R\) be an increasing CADLAG function. A Borel function \(f:(0,\infty)\to\mathbb R\) is \(a\)-integrable if}
...
then we can write this on \textbf{differential form}\index{differential form}

\[
df(t)=\varphi(u)\ dx(u),\qquad f(0)=f_0.
\]

This notation is equivalent with the integral above but is often easier to write down. We also say the \(f\) has \emph{dynamics}\index{dynamics} \(\varphi(u)\ dx(u)\).
...
\textbf{Theorem 4.1. (Lund) (Integration by parts formula)}\index{Integration by parts formula} \emph{If \(x\) and \(y\) are two FV-functions then the function \(z(t)=x(t)y(t)\) is given by}
...
\textbf{Theorem 4.2. (Lund) (Change of variable formula --- Ito formula for FV-functions)}\index{Ito formula}\index{Change of variable formula} \emph{Let \(f\) be a \(C^1\)-function and let \(x(t)\) be a FV-function. Then \(z(t)=f(x(t))\) is an FV-function and}
...
\textbf{Definition 6.1. (Lund) (Continuous-time stochastic process)}\index{Continuous-time stochastic process} \emph{A continuous-time stochastic process with state space \(\mathcal X\) is a family of stochastic variables \((X(t),t \ge 0)\) where \(X(t)\) takes values in \(\mathcal X\) for all \(t \ge 0\).}
...
\textbf{Definition 6.2. (Lund) (Filtration)}\index{Filtration} \emph{A filtration \(\mathcal F(t)\) is an increasing family of sub-\(\sigma\)-algebras of \(\mathcal F\). That is, for each \(t\) there is a \(\sigma\)-algebra \(\mathcal F(t)\) and \(\mathcal F(s)\subseteq \mathcal F(t)\) for \(0\le s<t<\infty\). Moreover}
...
\textbf{Definition 6.4. (Lund) (Adapted process)}\index{Adapted process} \emph{A stochastic process \(X(t)\) is adapted to the filtration \(\mathcal F(t)\) if, \(X(t)\) is an \(\mathcal F(t)\)-measurable stochastic variable for each \(t\).}
...
\textbf{Definition 6.5. (Lund) (Stopping time)}\index{Stopping time} \emph{A \([0,\infty]\)-valued random variable \(\tau\) is a stopping time with respect to the filtration \(\mathcal F(t)\) if}
...
\textbf{Definition 6.12. (Lund) (Elementary predictable sets)}\index{Elementary predictable sets} \emph{Let \(\mathcal F(t)\) be a filtration and let the elementary predictable sets be}
...
\NormalTok{  results[i}\SpecialCharTok{+}\DecValTok{1}\NormalTok{,}\StringTok{"Z"}\NormalTok{] }\OtherTok{\textless{}{-}}\NormalTok{ Z\_new}
  \ControlFlowTok{if}\NormalTok{ (Z\_new }\SpecialCharTok{!=}\NormalTok{ Z) \{}
\NormalTok{    plot[j,}\StringTok{"tend"}\NormalTok{] }\OtherTok{\textless{}{-}}\NormalTok{ t}\SpecialCharTok{+}\NormalTok{h}
\NormalTok{    j }\OtherTok{\textless{}{-}}\NormalTok{ j }\SpecialCharTok{+} \DecValTok{1} 
\NormalTok{    plot[j,] }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(t}\SpecialCharTok{+}\NormalTok{h,Z\_new,}\ConstantTok{NA}\NormalTok{,Z\_new)}
\NormalTok{  \}}
\NormalTok{  Z }\OtherTok{\textless{}{-}}\NormalTok{ Z\_new}
\NormalTok{\}}
\NormalTok{plot[j,}\StringTok{"tend"}\NormalTok{] }\OtherTok{\textless{}{-}} \DecValTok{120}
\FunctionTok{sum}\NormalTok{(results}\SpecialCharTok{$}\NormalTok{Z }\SpecialCharTok{==} \DecValTok{1}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{sum}\NormalTok{(results}\SpecialCharTok{$}\NormalTok{Z }\SpecialCharTok{==} \DecValTok{2}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{sum}\NormalTok{(results}\SpecialCharTok{$}\NormalTok{Z }\SpecialCharTok{==} \DecValTok{3}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\textbf{Corollary.} \emph{(Equivalence premium)\index{Equivalence premium} Assume that \(\mathbf R(t)=\mathbf R(t;\theta)\) for \(\theta\in\Theta\). Let \(\tilde\theta\) b the equivalence premium i.e.~\(\tilde \theta\) solves \(V^{Th}_{X(0)}(0;\tilde\theta)=0\), then the sequence \(\{\theta_n\}_{n\in \mathbb N_0}\) defined by}
...
\textbf{Theorem. (Thiele differential equation)}\index{Thiele differential equation} \emph{Let \(B\) denote the accumulated payments with dynamics}
\begin{align*}
...
\emph{``Part 2.b''} \emph{(Martingale approach)} Consider the Doob-meyer martingale
\begin{align*}
...
\textbf{Proposition 4.2. (Asmussen)} \emph{Assuming the with-profit model, the surplus increases with}
\begin{align*}
...
\end{align*}
\emph{In the above \(C\) is called the \textbf{surplus contribution process}.}
...
\textbf{Proposition 1.1. (\(\rho\)-modified Kolmogorov)}\index{$\rho$-modified Kolmogorov} \emph{Let the \(\rho\)-modified transition probabilities be given by (1). For \(i,j\in\mathcal J\) we have}
...
\textbf{Definition 2.6. (Basic Bi-Temporal Model)}\index{Basic Bi-Temporal Model} \emph{We say that \((X_t,Z_t,(H^t_s)_{0\le s\le t})_{t\ge 0}\) satisfy the basic bi-temporal assumptions if}
...
\textbf{Definition 2.1.1. (Mikosch) (Poisson process)}\index{Poisson process} \emph{A stochastic process \(N=(N(t))_{t\ge 0}\) is said to be a Poisson process if the following conditions hold:}
...
\textbf{Definition. (Mikosch) (Homogeneous Poisson Process)}\index{Homogeneous Poisson Process} \emph{Consider a Poisson process as in definition 2.1.1. If \(N(t)\) has mean value function}
...
\textbf{Lemma. (Mikosch)} \emph{If \(N\) is a homogeneous Poisson process with intensity \(\lambda\), then it holds that: \(N\) has CADLAG sample paths, starts at zero, has independent and \textbf{stationary increments}\index{stationary increments}, that is \(\mu(s,t]=\mu(s+h,t+h]\) for any \(h>0\), and \(N(t)\sim \text{Pois}(\lambda t)\) for every \(t>0\). If the first three properties are fulfilled we say the process is a \textbf{Levy process}\index{Levy process}.}

\textbf{Definition. (Mikosch) (Intensity function)}\index{Intensity function} \emph{Consider a Poisson process as in definition 2.1.1. We say \(N(t)\) has intensity function or rate function \(\lambda\) if \(\mu\) is absolutely continuous if for any \(0\le s <t<\infty\) it holds that}
...
\textbf{Theorem 2.1.6. (Mikosch)} \emph{Let \(W_1,W_2,...\) be iid \(\text{Exp}(\lambda)\) and define the random walk \(T_n=\sum_{i=1}^n W_i\) and initial value \(T_0=0\).}\index{renewal process}
...
\textbf{Proposition 2.1.8. (Mikosch) (Joint distribution of arrival/inter-arrival times)}\index{inter-arrival times} \emph{Assume \(N\) is a Poisson process on \([0, \infty)\) with a continuous a.e. positive intensity function \(\lambda\). Then the following statements hold.}
...
\textbf{Lemma 2.1.9. (Mikosch) (Joint density of order statistics)}\index{order statistics} \emph{Let \(X_1,X_2,...,X_n\) be iid, then the density of the order statistic \((X_{(1)},...,X_{(n)})\) is given by}
...
\textbf{Definition 1.1. (Schmidli) (Stochastic process)} \emph{A \textbf{stochastic process}\index{stochastic process} is a family of random variables \(X_t : (\Omega,\mathbb F,\mathbb P) \to (E,\mathbb E,\mu)\) indexed by \(t\in \mathcal I\), where \(\mathcal I\subseteq \mathbb N\) or \(\mathcal I\subseteq \mathbb R\), denoted by \(\mathbf X=\{X_t\}_{t\in \mathcal I}\). In the case that \(\mathcal I=\mathbb R\), we say that \(\mathbf X\) is \textbf{CADLAG}\index{CADLAG stochastic process} if}

\[
X(t)=\lim_{h\to 0}X(t+h)
\]

\emph{and \(\lim_{h\to 0}X(t-h)\) exist almost surely. Furthermore, if \(X(t)\) is continuous almost surely we say that \(\mathbf X\) is \textbf{continuous}\index{continuous stochastic process}.}
...
\textbf{Definition 1.2. (Schmidli) (Point process)} \emph{A CADLAG stochastic process \(\{N_t\}\) is called a \textbf{point process}\index{point process} if i) \(N_0=0\), ii) \(N_t\ge N_s\) for all \(t\ge s\) (increasing) and iii) \(N_t\in \mathbb N\) for all \(t\). Furthermore, if for the mapping \(T_k : k\mapsto\inf\{t\ge 0 : N_t\ge k\}\) (jumping times) \(T_k\ne T_j\) for all \(k\ne j\), then \(\mathbf N\) is said to be a \textbf{simple point process}.\index{simple point process}}

\textbf{Definition 1.3. (Schmidli)} \emph{Let \(\{X_t\}\) be a stochastic process. If for all partitions \(0=t_0<t_1<\cdots<t_n\) the random variables \(X_{t_i}-X_{t_{i-1}}\) for \(i=1,...,n\) are jointly independent, then \(\{X_t\}\) has \textbf{independent increments}\index{independent increments}. If furthermore for all \(h>0\) it holds that}

\[
\{X_{t_i}-X_{t_{i-1}}\}_{i=1,...n}\stackrel{d}{=}\{X_{t_i+h}-X_{t_{i-1}+h}\}_{i=1,...n}
\]

\emph{then \(\{X_t\}\) hast \textbf{stationary increments}\index{stationary increments}.}
...
\textbf{Definition 1.9. (Schmidli) (Poisson process)} \emph{A point process \(\{N_t\}\) is called a (homogeneous) \textbf{Poisson process}\index{poisson process} with rate \(\lambda >0\) if i) \(\{N_t\}\) has stationary and independent increments and ii) \(\mathbb P[N_j=0]=1-\lambda h+o(h)\) as \(h\to 0\).}

\textbf{Proposition 1.10. (Schmidli) (Poisson process, alternative version)} \emph{A point process \(\{N_t\}\) is a (homogeneous) \textbf{Poisson process}\index{poisson process} with rate \(\lambda >0\) if one of the following statements is true:}
...
\textbf{Definition 1.12. (Schmidli) (inhomogeneous Poisson process)}\index{inhomogeneous Poisson process} \emph{Let \(\Lambda(t)\) be an increasing right continuous function on \([0,\infty)\) with \(\Lambda(0) = 0\). A point process \(\{N_t\}\) on \([0, \infty)\) is called \textbf{inhomogeneous Poisson process} with intensity measure \(\Lambda(t)\) if}
...
\textbf{Definition 1.15. (Schmidli) (Renewal process)}\index{Renewal process} \emph{A simple point process \(\{N_t\}\) is called an \textbf{ordinary renewal process} if the interarrival times \(\{T_k-T_{k-1}\}_{k\in\mathbb N}\) are iid. If \(T_1-T_0=T_1\) has a different distribution then \(\{N_t\}\) is called \textbf{delayed renewal process}. If \(\frac{1}{\lambda}=\mathbb E[T_2-T_1]\) exist and}

\[
\mathbb P[T_1\le x]=\lambda\int_0^x\mathbb P[T_1-T_1>y]\ dy
\]

\emph{then \(\{N_t\}\) is called \textbf{stationary renewal process}. If \(\{N_t\}\) is an ordinary renewal process then the function \(U(t)=1_{\{t\ge 0\}}+\mathbb E[N_t]\) is called the \textbf{renewal measure}.}\index{renewal measure}
...
where \(z(x)\) is a known function and \(Z(x)\) is unknown. This equation is called the \textbf{renewal equation}\index{renewal equation}. The equation can be solved explicitly.
...
\textbf{Definition 1.20. (Schmidli)}\index{arithmetic distribution} \emph{Let \(F\) be a distribution function for a random variable \(X\). If \(\gamma\) exists such that}
...
which is called the \textbf{Wierner-Hopf factorisation}.\index{Wierner-Hopf factorisation}

\hypertarget{subexponential-distributions}{%
\subsection{Subexponential distributions}\label{subexponential-distributions}}

\textbf{Definition 1.28. (Schmidli) (Subexponential distribution)}\index{Subexponential distribution} \emph{A distribution function \(F\) with \(F(x)=0\) for \(x<0\) is called \textbf{subexponential} if}
...
The condition can be interpreted that the mean income is strictly larger than the mean outflow. Therefore the condition is also called the \textbf{net profit condition}\index{net profit condition}.
...
\textbf{Theorem 2.6. (Schmidli) (Cramer-Lundberg approximation)}\index{Cramer-Lundberg approximation} \emph{Assume that the Lundberg exponent exists and that}
...
\end{align*}
The random variable \(−C_\tau\) is called \textbf{severity of ruin}\index{severity of ruin}.
...
\textbf{Definition 2.10. (Schmidli) (Laplace transform)}\index{Laplace transform} \emph{Let \(f\) be a real valued function on \([0,\infty)\). The transform}
...
\textbf{Definition. (Binomial distribution)}\index{Binomial distribution} \emph{Let \(B_1,...,B_n\) be i.i.d \(\text{Bern}(p)\) distributed. The quantity \(B=\sum_{i=1}^n B_i\) is said to follow a Binomial distrbution with \(n\) trials and success probability \(p\) written \(B\sim \text{Bin}(n,p)\). The point probabilities are}
...
\textbf{Definition. (Mixture distribution)}\index{Mixture distribution} \emph{Let \(X\) and \(W\) be two random variables with distribution functions \(F_X\) and \(F_W\). If}
...
\textbf{Definition 4.1. (Mixed exponential distributions)}\index{Mixed exponential distributions} \emph{If each \(X_i\sim \exp(\beta_i)\) for \(i=1,...,K\) in a finite mixture, we write \(X\sim \text{mexp}(\beta;p)\), where \(\beta=(\beta_1,...,\beta_K)\) and \(p=(p_1,...,p_{K})\) with \(\sum_{i=1}^Kp_i=1\). That is}
...
\textbf{Definition. (Thinning)}\index{Thinning} \emph{Let \(N^*\) be a random variable on \(\{0,1,...,\}=\mathbb N_0\) and let \((I_i)_{i\in\mathbb N_0}\) be i.i.d \(\text{Bern}(q)\) variables. The variable \(N\) defined as}
...
\hypertarget{linear-smoothers}{%
\subsection{Linear Smoothers}\label{linear-smoothers}}
...
\textbf{Definition. (Kernel Smoother)}\index{Kernel Smoother} \emph{The kernel smoother is a linear smoother with }
...
\textbf{Definition. (Splines)}\index{Splines} \emph{Consider a function \(S : \mathbb R\to \mathbb R\). If}
...
\textbf{Definition. (Natural Splines)}\index{Natural Splines} \emph{Consider a \(k\)th order spline \(S\), \(k\) odd, with smoothness \(k-1\) defined for the knotpoints \(t_0,...,t_\ell\). We say that \(S\) is a natural spline of order \(k\) if \(S\) is a \(C^{(k-1)/2}\) on \((-\infty,t_0)\cup (t_\ell,\infty)\).}
...
\textbf{Definition. (CART)}\index{CART} \emph{Start with \(\mathcal X\) as initial node for splitting. Follow the process below for all subnotes made from splitting.}
...
\textbf{Definition. (Subtree)}\index{subtree} \emph{We call \(T'\) a subtree of \(T\) if the nodes of \(T'\) is a subset of the nodes of \(T\). \(T'\) and \(T\) have the same root. If \(T'\) is a subtree of \(T\), we also write \(T'\subseteq T\).}

We can derive a subtree, by pruning a tree at any non-terminal node (i.e.~by deleting all descendants of that node)

\textbf{Definition. (\(\alpha\)-pruned tree)}\index{$\alpha$-pruned tree} \emph{Consider a tree \(\hat T_n\) with corresponding estimator \(\hat m_n(\hat T_n)\). Given a parameter \(\alpha>0\), the \(\alpha\)-pruned tree is}
...
\textbf{Proposition (weakest link)}\index{weakest link algorithm} \emph{The \(\alpha\)-pruned tree \(\hat T_{n,\alpha}\) is unique. Furthermore, there exist (unique) trees \(T_0\supset \cdots\supset T_l\) with \(\{T_0,\dots, T_l\}=\{\hat T_{n,\alpha} : \alpha>0\}:=\hat{ \mathbb T}_n\); in particular the set \(\{\hat T_{n,\alpha} : \alpha>0\}\) is finite.}
...
\textbf{Definition.} \emph{(Bagging)}\index{Bagging} \emph{Given data \({(X_1,Y_1),\dots, (X_n,Y_n)}\), draw \(B\) bootstrap samples \(\tilde {\mathcal D}^{(b)}_n={(\tilde X^{(b)}_1,\tilde Y^{(b)}_1),\dots, (\tilde X^{(b)}_n,\tilde Y^{(b)}_n)}\), for \(b=1,\dots, B\), i.e., \(\tilde {\mathcal D}^{(b)}_n\) arises from \(\mathcal D_n\) by drawing \(n\) times with replacement. The bagging estimator is defined as the following. In the squared loss case (average):}
...
\textbf{Gradient Boosting (Second version)} \emph{XGBoost}\index{XGBoost} Use a second order approximation of \(\hat R_{n,\gamma,\lambda}\)
\begin{align}
...
\includegraphics{theory_files/figure-latex/unnamed-chunk-21-1.pdf}
...
\includegraphics{theory_files/figure-latex/unnamed-chunk-22-1.pdf}
...
\hypertarget{standard-distributions-1}{%
\section{Standard distributions}\label{standard-distributions-1}}
...
\begin{center}\includegraphics[width=0.25\linewidth]{theory_files/figure-latex/unnamed-chunk-27-1} \end{center}
...
\begin{center}\includegraphics[width=0.4\linewidth]{theory_files/figure-latex/unnamed-chunk-28-1} \end{center}
...
Obviously, we may introduce the \textbf{natural filtration}\index{natural filtration} \(\mathcal{F}^X_t\) given by the trajectory of the process \(X_t\):
...
\textbf{Proposition 1.7. (Schmidli) (Martingale stopping theorem)}\index{Martingale stopping theorem} \emph{Let \(M_t\) be an martingale wrt. a filtration \(\mathcal F_t\). Assume that \(T\) is a \(\mathcal F_t\) stopping time. If \(\mathcal F_t\) is continuous, then \(M_{\min\{T,t\}}\) is also a \(\mathcal F_t\) martingale. Furthermore, \(\mathbb E[M_t\ \vert\ \mathcal F_T]=M_{\min\{T,t\}}\)}

\textbf{Proposition 1.8. (Schmidli) (Martingale convergence theorem)}\index{Martingale convergence theorem} \emph{Let \(M_t\) be an martingale wrt. a filtration \(\mathcal F_t\). If \(\sup\mathbb E\vert M_t\vert <\infty\), then \(M_\infty:=\lim_{t\to\infty} M_t\) exists and \(\mathbb E\vert M_t\vert<\infty\).}

...
\makeatother
\subtitle{Compilation of theory}
\author{Joakim Bilyk}
...
\textsc{\Large Compilation of
theory}\\[0.5cm] % Major heading such as course name
...
\includegraphics{/Users/joakimbilyk/Documents/GitHub/pdf/render/theory_files/figure-latex/unnamed-chunk-3-1.pdf}
...
\begin{wrapfigure}{R}{0.5\textwidth}\includegraphics{/Users/joakimbilyk/Documents/GitHub/pdf/render/theory_files/figure-latex/unnamed-chunk-5-1}\end{wrapfigure}
...
\tightlist
\item
  \emph{There exists an \(n\times n\) matrix \(B\) such that
  \(AB=I_n=BA\).}
\item
  \emph{There exist either a left inverse \(B\) or a right invers \(C\)
  i.e.~\(BA=I_n=AC\). In this case, \(B=C\).}
\item
...
\item
  \emph{The equation \(Ax=0\) (\(x\in K\)) has only the trivial solution
  \(x=0\).}
\item
...
\item
  \emph{The linear transformation \(Ax\) is a bijection from \(K\) to
  \(K\).}
\item
...
\item
  \emph{\(A\) can be expressed as a finite product of elemtary
  matrices.}
\end{enumerate}

We futhermore have some properties.

\textbf{Proposition.} \textbf{(Properties)} \emph{Let \(A\) be an
\(n\times n\) invertible matrix. Then}
...
\item
  \((Ax)^+=x^+A^{-1}\) \emph{if \(A\) has orthonormal columns.
  \((\cdot)^+\) denotes the Moore-Penrose inverse and \(x\) is a
  vector.}
\item
  \emph{If \(B\) is an \(n\times n\) invertible matrix then
  \((AB)^{-1}=B^{-1}A^{-1}\).}
\item
...
\emph{where \(Z\) is allowed to vary over the class of all square
integrable \(\mathcal{G}\) measurable random variables. The optimal
solution \(\hat{Z}\) is then given by.}
